...
这是一个根据 LangChain 1.2 课程代码整理的教学型私人关系记忆助手。它可以分别管理妈妈、爸爸、姐姐、恋人、朋友等关系对象，检索私人备忘录、记住用户确认过的偏好，并在需要时调用实时工具。恋爱助手是其中的一个使用场景。
...
`knowledge/` 默认保持为空，用户可以在网页中上传 `.txt`/`.md` 关系笔记，也可以在聊天中补充姓名、生日、喜好、雷区和重要日期。前端通过 `subject_id` 指定当前对象，长期记忆按 `user_id + subject_id` 隔离。聊天提取到的新信息应先由用户确认，再写入长期记忆和 RAG 索引。
...
基于 LangChain 1.2 与 FastAPI 开发隐私优先的私人关系记忆助手，基于 `subject_id` 隔离家人、恋人和朋友资料，整合关系备忘录、RAG 检索、工具调用、SSE 流式响应和长短期记忆；通过结构化输出提取对象喜好与雷区，并支持内存/PostgreSQL 记忆和内存/Milvus 向量检索切换。
